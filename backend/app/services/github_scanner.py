...
REGEX_RULES = [
    ...
    {"id": "CFG003", "title": "Hardcoded Firebase / Google API Key", "severity": "high",
     ...
     "description": "Firebase or Google API key found hardcoded. Restrict its usage in the Firebase/Google Cloud console and move it to environment variables."},
    ...
    {"id": "CRY005", "title": "Non-Cryptographic Random for Security", "severity": "medium",
     ...
     "description": "Math.random() / random.random() are not cryptographically secure. Use crypto.getRandomValues() or secrets.token_hex() for security-sensitive values."},
    # ── Injection ─────────────────────────────────────────────────────────────
...
RECOMMENDATIONS = {
    ...
    "JS004": "Remove or guard all `console.log` statements containing sensitive variables before deploying to production. Use a logging library that supports log level control and redaction. In React Native, use Flipper or a crash reporter with PII filtering.",
    ...
    "JS006": "Never disable TLS certificate verification in production. Remove `rejectUnauthorized: false` and fix the underlying certificate issue. Use a CA-signed certificate and test with `curl --cacert ca.pem` to verify the chain.",
    "JS007": "Replace `document.write()` with modern DOM insertion methods: `element.insertAdjacentHTML('beforeend', DOMPurify.sanitize(data))`. Never pass user-controlled data to `document.write()`.",
    "CFG003": "Move Firebase/Google API keys to environment variables: `process.env.FIREBASE_API_KEY`. Restrict API key usage in the Firebase console (referrer, IP, or bundle ID restrictions). For sensitive APIs, use service account keys stored in a secrets manager.",
    "CFG004": "Remove the connection string immediately and rotate the database password. Store credentials in environment variables and use a secrets manager (AWS Secrets Manager, HashiCorp Vault). Audit Git history with `git log -S 'password' --all` and purge with `git filter-repo`.",
    "CFG005": "Generate a strong JWT secret: `python -c \"import secrets; print(secrets.token_hex(64))\"`. Store it as an environment variable: `JWT_SECRET = os.getenv('JWT_SECRET')`. Rotate it if it has been exposed. Consider using RS256 (asymmetric) instead of HS256.",
    "NET002": "Set `android:usesCleartextTraffic=\"false\"` in `AndroidManifest.xml`. If you need HTTP for specific domains (e.g., localhost), use a `network_security_config.xml` to whitelist only those domains. All production traffic should use HTTPS.",
    "NET003": "Replace `Allow-Origin: *` with an explicit list of trusted origins: `Access-Control-Allow-Origin: https://yourapp.example.com`. In Express: `app.use(cors({ origin: ['https://yourapp.com'] }))`. Wild-card CORS combined with cookies allows CSRF.",
    "NET004": "Replace hardcoded IP addresses with DNS hostnames and store them in environment variables: `API_HOST=os.getenv('API_HOST')`. This improves portability and allows infrastructure changes without code deployments.",
    "CRY003": "Replace DES/3DES with AES-256-GCM. Python: `from cryptography.hazmat.primitives.ciphers.aead import AESGCM`. Java: `Cipher.getInstance(\"AES/GCM/NoPadding\")`. 3DES offers only ~112-bit effective security and is deprecated by NIST since 2023.",
    "CRY004": "Replace ECB mode with AES-GCM (authenticated encryption): Python: `AESGCM(key).encrypt(nonce, data, None)`. ECB mode is deterministic — identical plaintext blocks produce identical ciphertext blocks, leaking data patterns.",
    "CRY005": "Use a cryptographically secure random generator. JavaScript: `crypto.getRandomValues(new Uint8Array(32))` or `crypto.randomBytes(32)`. Python: `secrets.token_hex(32)` or `os.urandom(32)`. Never derive tokens, salts, or nonces from `Math.random()`.",
    "INJ001": "Validate and canonicalize all file paths derived from user input. Python: `safe = base_path / pathlib.Path(user_input).name` (strips directory components). Reject paths containing `..`, `~`, or absolute separators. Use allow-lists for expected filenames.",
    "INJ002": "Never pass user input to `render_template_string()` or `Template()`. Use static template files with Jinja2/Twig and pass data as context variables: `render_template('page.html', name=user_value)`. Set `autoescape=True` in all template engines.",
    "SEC005": "Remove credentials files from the repository immediately. Run `git filter-repo --path kubeconfig --invert-paths` to purge from history. Rotate all affected credentials. Use `KUBECONFIG` environment variable and store the file outside the repo.",
}
